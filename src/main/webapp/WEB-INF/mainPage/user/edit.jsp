  ...
  <div class="body-content">
    <form name="form" action="${pageContext.request.contextPath}/user_update.html" class="form-x" enctype="multipart/form-data" method="post" onsubmit="return check(this)">
	      ...
	      <div class="form-group">
	        ...
	        <div class="field">
	          <input type="hidden" id="sex" value="${user.user_sex}">
	          <select name="user_sex" id="user_sex" class="input w50">
		  ...
		  <div class="form-group">
	        ...
	        <div class="field">
	          <input type="text" class="input w50" value="${user.user_phone}" name="user_phone" data-validate="tel:手机格式不正确" />
	        </div>
	      </div>
	      <div class="form-group">
	        <div class="label">
	      ...
	      <div class="form-group">
	       ...
	       <div class="form-group">
	         ...
	         <div class="field" style="padding-top:8px;"> 
	         <input type="hidden" name="login_ip" value="${user.login_ip}">
	         ${user.login_ip}  <input type="button" onclick="return ipInfo('${user.login_ip}')" value="IP详情">
	         </div>
	       </div>
	       <div class="form-group">
	         <div class="label">
	         ...
	         <div class="field" style="padding-top:8px;">
	            ...
	            <select name="user_state" id="user_state" class="input w50">
		          ...
		          <s:elseif test="#session.user.user_state==1">
		          	  <option value="1">普通用户</option>
		          </s:elseif>
		          <s:elseif test="#session.user.user_state==2">
		          	  <option value="2">禁言</option>
		          </s:elseif>
		          <s:elseif test="#session.user.user_state==3">
		          	  <option value="3">禁言禁发布</option>
		          </s:elseif>
	            </select>
	         </div>
...
function showImg(file) {
	document.getElmentById("img").src = window.URL.createObjectURL(file.files[0]);
}
//state
document.getElementById("user_state").value = document.getElementById("state").value;
//sex
document.getElementById("user_sex").value = document.getElementById("sex").value;
//查看ip信息
function ipInfo(ip){
    window.location.href="http://ip.chinaz.com/"+ip;
}
</script>