	...
	<table class="table table-hover text-center">
		...
		<tr>
			...
			<td colspan="10" style="text-align: left; padding-left: 20px;"><a
				href="javascript:void(0)" class="button border-red icon-trash-o" onclick="batchDelete()"
		...
		<tr>
			<td colspan="11"><div class="pagelist">
					<s:if test="#pageBean.current_page != 1">
						<a
							href="${pageContext.request.contextPath}/news_findAll.html?current_page=1&user_news_id=${pageBean.list[0].user.user_id}">首页</a>
					</s:if>
					<s:if test="#pageBean.current_page > 1">
						<a
							href="${pageContext.request.contextPath}/news_findAll.html?current_page=${pageBean.current_page-1}&user_news_id=${pageBean.list[0].user.user_id}">上一页</a>
					</s:if>
					${pageBean.current_page}/${pageBean.total_page}
					<s:if test="#pageBean.current_page < #pageBean.total_page">
						<a
							href="${pageContext.request.contextPath}/news_findAll.html?current_page=${pageBean.current_page+1}&user_news_id=${pageBean.list[0].user.user_id}">下一页</a>
					</s:if>
					<s:if test="#pageBean.current_page != #pageBean.total_page">
						<a
							href="${pageContext.request.contextPath}/news_findAll.html?current_page=${pageBean.total_page}&user_news_id=${pageBean.list[0].user.user_id}">尾页</a>
					</s:if>
				</div></td>
		</tr>
	</table>
...
<script type="text/javascript">
	...
	function del_multi() {
		...
		window.location.href="news_delAll.html?ids="+x;
	}
</script>